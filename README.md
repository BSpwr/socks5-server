...
[![](https://github.com/bspwr/socks5-server/workflows/docker%20build/badge.svg)](https://github.com/bspwr/socks5-server/actions/workflows/deploy.yml)

This is my fork of [serjs/socks5-server](https://github.com/serjs/socks5-server).
...
- Run docker container using default container port 1080 and expose it to world using host port 1080, with auth creds
    ...
    ```docker run -d --name socks5 -p 1080:1080 -e PROXY_USER=<PROXY_USER> -e PROXY_PASSWORD=<PROXY_PASSWORD>  ghcr.io/bspwr/socks5-server```
...
- Run docker container using specifit container port and expose it to host port 1090, without auth creds
    ...
    ```docker run -d --name socks5 -p 1090:9090 -e PROXY_PORT=9090 ghcr.io/bspwr/socks5-server```
